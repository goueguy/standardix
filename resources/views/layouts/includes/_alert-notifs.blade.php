        ...
        <span class="flex justify-end mr-6 flex-end">
            <svg class="w-10 h-10 text-opacity-100 text-medium" viewBox="0 0 88 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                @if(!auth()->user()->unreadNotifications->isEmpty())
                    <span class="absolute flex items-center w-5 h-5 p-1 my-1 mt-1 text-xs bg-red-700 border-2 border-white rounded-full z-2 justify-content-center"><span class="font-bold text-white">{{auth()->user()->unreadNotifications->count()}}</span></span>
                @else
